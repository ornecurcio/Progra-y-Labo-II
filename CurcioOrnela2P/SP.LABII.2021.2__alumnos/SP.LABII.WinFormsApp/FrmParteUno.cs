...
namespace SP.LABII.WinFormsApp
{
    ...
    public partial class FrmParteUno : Form
    {
        ...
        private void btnPunto4_Click(object sender, EventArgs e)
        {
            ...
            this.c_fosforos.PrecioSuperado += Caja_EventoPrecio;

            this.c_fosforos += new Fosforo("Madera", "Fragata", 1000, 7);
        }

        private void Caja_EventoPrecio(object sender, EventArgs e)
        {
            bool todoOK = Facturadora<Fosforo>.ImprimirTicket(this.c_fosforos);///Reemplazar por la llamada al método de clase ImprimirFactura
        ...
        private void btnPunto5_Click(object sender, EventArgs e)
        {
            ...
            if (((IDeserealiza)this.zapato).Xml(out aux))
            {
        ...
        private void btnHilos_Click(object sender, EventArgs e)
        {
            Task hilo = Task.Run(() =>
            {
                if (this.lstVisor.InvokeRequired) // DA TRUE CUANDO NO ES DEL HILO PRINCIPAL 
                {
                    this.lstVisor.BeginInvoke((MethodInvoker)delegate ()
                    {
                        Thread.Sleep(3000);
